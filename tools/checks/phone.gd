...
## Every phone contact and special call on all three cartridges. `PhoneContacts`
## and `SpecialPhoneCallList` are reached by routines rather than by a script the
## catalog walks, so a row the importer read short reaches a silent phone.
