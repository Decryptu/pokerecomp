...
func _dex(seen: Array, caught: Array = [], mode: int = Gen2Layout.DEXMODE_NEW,
...
func test_new_mode_lists_the_new_dex_table_and_old_mode_the_species_range() -> void:
	var new_dex: Gen2Pokedex = _dex([Gen2Layout.SPECIES_COUNT])
	assert_eq(new_dex.rows()[0]["species"], Gen2Layout.SPECIES_COUNT)

	var old_dex: Gen2Pokedex = _dex([1], [], Gen2Layout.DEXMODE_OLD)
	var species: Array = []
...
func test_abc_mode_lists_only_seen_species_and_zero_fills_the_tail() -> void:
	...
	var dex: Gen2Pokedex = _dex([3, 4, 8], [], Gen2Layout.DEXMODE_ABC)
...
func test_a_row_shows_a_name_only_once_seen_and_a_symbol_only_once_caught() -> void:
	var dex: Gen2Pokedex = _dex([251, 250], [250], Gen2Layout.DEXMODE_OLD)
	dex.scroll = 249
	var rows: Array = dex.rows()
	...
	var unseen: Gen2Pokedex = _dex([251], [], Gen2Layout.DEXMODE_OLD)
...
func test_only_old_mode_numbers_its_rows() -> void:
	assert_eq(String(_dex([1], [], Gen2Layout.DEXMODE_OLD).rows()[0]["number"]), "001")
...
func test_the_cursor_moves_then_scrolls_and_stops_at_the_end() -> void:
	var dex: Gen2Pokedex = _dex([1], [], Gen2Layout.DEXMODE_OLD)
	dex.listing_end = 10
	for step: int in Gen2Pokedex.LISTING_HEIGHT - 1:
		assert_true(dex.move_listing(PokeButton.DOWN))
	...
	assert_false(dex.move_listing(PokeButton.DOWN), "the listing ends at 10")
...
func test_moving_up_empties_the_cursor_before_it_scrolls() -> void:
	...
	dex.cursor = 2
	dex.scroll = 5
	assert_true(dex.move_listing(PokeButton.UP))
	...
	dex.scroll = 0
	assert_false(dex.move_listing(PokeButton.UP), "nothing above the top")
...
func test_paging_is_refused_while_the_listing_fits_on_one_screen() -> void:
	...
	assert_false(dex.move_listing(PokeButton.RIGHT))
	assert_false(dex.move_listing(PokeButton.LEFT))
	assert_eq(dex.scroll, 0)
...
func test_a_page_moves_a_screen_and_clamps_to_either_end() -> void:
	var dex: Gen2Pokedex = _dex([1], [], Gen2Layout.DEXMODE_OLD)
	dex.listing_end = 30
	assert_true(dex.move_listing(PokeButton.RIGHT))
	...
	assert_true(dex.move_listing(PokeButton.LEFT))
...
func test_the_entry_screen_opens_only_for_a_seen_species() -> void:
	var dex: Gen2Pokedex = _dex([250], [], Gen2Layout.DEXMODE_OLD)
...
func test_a_caught_entry_carries_its_measurements_and_both_pages() -> void:
	var dex: Gen2Pokedex = _dex([204], [204], Gen2Layout.DEXMODE_OLD)
	dex.scroll = 203
	var entry: Dictionary = dex.entry()
...
func test_a_zero_measurement_is_left_blank() -> void:
	...
	var dex: Gen2Pokedex = Gen2Pokedex.open(
		data, _state([1], [1]), Gen2Layout.DEXMODE_OLD
	)
	var entry: Dictionary = dex.entry()
	assert_eq(String(entry["height"]), "")
	assert_eq(String(entry["weight"]), "")
...
func test_stepping_through_entries_skips_unseen_species() -> void:
	var dex: Gen2Pokedex = _dex([1, 5], [], Gen2Layout.DEXMODE_OLD)
	dex.open_entry()
	assert_eq(int(dex.entry()["species"]), 1)
	assert_true(dex.step_entry(PokeButton.DOWN))
	assert_eq(int(dex.entry()["species"]), 5, "2 through 4 have not been seen")

	assert_false(dex.step_entry(PokeButton.DOWN), "nothing seen below 5")
...
func test_stepping_to_a_new_entry_returns_to_page_one() -> void:
	var dex: Gen2Pokedex = _dex([1, 2], [1, 2], Gen2Layout.DEXMODE_OLD)
	dex.open_entry()
	dex.toggle_page()
...
func test_the_dex_opens_on_the_last_entry_viewed() -> void:
	...
	for number: int in range(1, 21):
		seen.append(number)
	var dex: Gen2Pokedex = Gen2Pokedex.open(
		_data, _state(seen), Gen2Layout.DEXMODE_OLD, 12
	)
	assert_eq(dex.selected_species(), 12)


func test_no_previous_entry_opens_at_the_top() -> void:
	var dex: Gen2Pokedex = _dex([1, 2, 3], [], Gen2Layout.DEXMODE_OLD)
...
func test_changing_to_the_current_mode_changes_nothing() -> void:
	var dex: Gen2Pokedex = _dex([1], [], Gen2Layout.DEXMODE_OLD)
	dex.scroll = 3
	assert_false(dex.change_mode(Gen2Layout.DEXMODE_OLD))
...
func test_changing_mode_reorders_and_reseeks_the_previous_entry() -> void:
	...
	for number: int in range(1, 21):
		seen.append(number)
	var dex: Gen2Pokedex = Gen2Pokedex.open(_data, _state(seen), Gen2Layout.DEXMODE_OLD, 12)
	assert_eq(dex.selected_species(), 12)
	assert_true(dex.change_mode(Gen2Layout.DEXMODE_ABC))
	assert_eq(dex.mode, Gen2Layout.DEXMODE_ABC)
...
func test_the_type_rows_are_the_only_ones_left_and_right_change() -> void:
	...
	assert_false(dex.move_search_type(PokeButton.RIGHT))
	dex.search_cursor = Gen2Pokedex.SEARCH_ROW_TYPE_1
	assert_true(dex.move_search_type(PokeButton.RIGHT))
...
func test_a_search_row_names_its_type() -> void:
	...
	assert_eq(dex.search_type_name(1), _data.type_name(Gen2Layout.TYPE_NORMAL))
...
func test_a_search_finds_caught_species_only() -> void:
	...
	var dex: Gen2Pokedex = _dex([1, 18], [1], Gen2Layout.DEXMODE_OLD)
	dex.search_type_1 = 1
...
func test_two_types_narrow_the_search_rather_than_widening_it() -> void:
	...
	var dex: Gen2Pokedex = _dex(caught, caught, Gen2Layout.DEXMODE_OLD)
...
func test_unown_mode_is_unlocked_by_its_flag_and_lists_the_catching_order() -> void:
	...
	var dex: Gen2Pokedex = Gen2Pokedex.open(_data, state, Gen2Layout.DEXMODE_NEW)
...
func test_the_unown_cursor_stops_at_both_ends_of_what_has_been_caught() -> void:
	...
	var dex: Gen2Pokedex = Gen2Pokedex.open(_data, state, Gen2Layout.DEXMODE_NEW)
	dex.open_unown_mode()
	assert_false(dex.move_unown(PokeButton.LEFT), "the first form is the left edge")
	assert_false(dex.move_unown(PokeButton.UP), "up and down are not read at all")
	assert_true(dex.move_unown(PokeButton.RIGHT))
	assert_eq(dex.selected_unown_form(), 9)
	assert_eq(dex.unown_word(), "IWORD")
	assert_false(dex.move_unown(PokeButton.RIGHT), "the last form caught is the right edge")
...
func test_an_empty_unown_dex_has_no_form_and_no_word() -> void:
	...
	assert_false(dex.move_unown(PokeButton.RIGHT))
...
func test_a_mod_species_follows_the_cartridge_run_in_every_mode() -> void:
	...
	for mode: int in [Gen2Layout.DEXMODE_NEW, Gen2Layout.DEXMODE_OLD]:
		var dex: Gen2Pokedex = _dex([MOD_SPECIES], [MOD_SPECIES], mode)
		assert_eq(dex.listing_end, Gen2Layout.SPECIES_COUNT + 1,
	...
	var abc: Gen2Pokedex = _dex([4, MOD_SPECIES], [], Gen2Layout.DEXMODE_ABC)
...
func test_the_dex_reopens_on_a_mod_species_it_was_left_on() -> void:
	...
	_data.set_content_overlay(overlay)
	var dex: Gen2Pokedex = _dex([MOD_SPECIES], [], Gen2Layout.DEXMODE_OLD, MOD_SPECIES)
	assert_eq(dex.selected_species(), MOD_SPECIES)
	...
	var unknown: Gen2Pokedex = _dex(
		[MOD_SPECIES], [], Gen2Layout.DEXMODE_OLD, Gen2ContentOverlay.FIRST_MOD_NUMBER + 9
	)
...
func test_the_entry_screen_names_its_footprint_cells() -> void:
	...
	assert_eq(tiles[2], tiles[0] + Gen2Layout.FOOTPRINT_HALF_STRIDE)
...
func test_the_unown_screen_places_its_letters_and_word() -> void:
	...
	assert_eq(_cell(map, 4, 11), Gen2Layout.UNOWN_FONT_FIRST_TILE, "A caught first")
	assert_eq(
		_cell(map, 4, 10), Gen2Layout.UNOWN_FONT_FIRST_TILE + 8,
