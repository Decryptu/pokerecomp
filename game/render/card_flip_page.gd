...
## `CardFlip_CopyOAM` writes, node-free. Every object is eight by eight and wears
## the map's own palettes; Crystal's digits stand one pixel higher, from
## `CardFlip_ShiftDigitsUpOnePixel`, unreferenced in pokegold.
