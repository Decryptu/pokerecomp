...
const WORDING: Dictionary = {
	...
	&"invalid_mod_page": "A mod registered a page with no id.",
	&"mod_page_missing_callable": "%s's page has no rows to list.",
	&"duplicate_mod_page": "%s already has a page; a mod may register one.",
	&"empty_notice": "%s asked to show a notice with no words in it.",
	&"notice_line_too_long": "A notice line will not fit the banner (%s).",
	&"unknown_notice_sound": "%s asked for a sound a notice may not play.",
	&"invalid_notice_icon": "%s's notice icon is not a set of fields.",
	&"notice_queue_full": "%s asked for more notices than the map can show.",
	...
	&"duplicate_text": "Two mods both rewrote the same box (%s).",
	&"invalid_text_key": "A mod rewrote a box it did not name (%s).",
	&"empty_text": "%s was rewritten to say nothing.",
