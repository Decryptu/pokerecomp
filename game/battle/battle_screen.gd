...
## one at a time with every number read from the event. Drawing is a registered renderer (the
## overworld map's boundary) with the text box in hardware pixels over it.
...
## `BattlePack` is the pack's own screen, opened over the fight.
...
const GHOST_NAME: String = "GHOST"
const GHOST_PIC: String = "ghost"
const GHOST_GET_OUT_TEXT: String = "GHOST: Get out...\nGet out..."
const GHOST_CANT_BE_IDD_TEXT: String = "Darn! The GHOST\ncan't be ID'd!"
const GHOST_UNVEILED_TEXT: String = "SILPH SCOPE\nunveiled the" + SCROLL + "GHOST's identity!"
## `MarowakAnim`: eight flashes, three shifts out, four shifts in, all ten
## frames apart, and the `Delay3` behind them.
const UNVEIL_STEP_FRAMES: int = 10
const UNVEIL_FLASHES: int = 8
const UNVEIL_FADE_STEPS: int = 3
const UNVEIL_FADE_IN_STEPS: int = 4
const UNVEIL_FLASH_MASK: int = 0x80
const UNVEIL_FRAMES: int = UNVEIL_STEP_FRAMES * (UNVEIL_FLASHES + UNVEIL_FADE_STEPS + UNVEIL_FADE_IN_STEPS) + 3
const GEN1_INFLICTED: Dictionary = {
	&"sleep": "\nfell asleep!", &"poison": "\nwas poisoned!", &"toxic": "'s\nbadly poisoned!",
	&"burn": "\nwas burned!", &"freeze": "\nwas frozen solid!",
	&"paralysis": "'s\nparalyzed! It may" + SCROLL + "not attack!",
}
const GEN1_HURT_BY: Dictionary = {"poison": "'s\nhurt by poison!", "burn": "'s\nhurt by the burn!"}
const GEN1_STAT_NAMES: Dictionary = {"special": "SPECIAL", "evasion": "EVADE"}
const GEN1_SEND_OUT_LINES: Array[String] = [
	"Go! %s!", "Do it! %s!", "Get'm! %s!", "The enemy's weak!\nGet'm! %s!",
]
const GEN1_SCREEN_SET_TEXT: Dictionary = {
	Gen2Screens.LIGHT_SCREEN: "%s's\nprotected against" + SCROLL + "special attacks!",
	Gen2Screens.REFLECT: "%s\ngained armor!",
}
const SCROLL: String = Gen2TextStream.SCROLL_BREAK
const PAGE: String = Gen2TextStream.PAGE_BREAK
...
## The word the games print for a stat, keyed as [Gen2BattleMon] keeps it. An unknown key
## prints as its snake_case name in capitals, as [method _battler_name] does for a status.
...
var _gen1_blackout_shown: bool = false
var _gen1_black: bool = false
## `IsGhostBattle`'s two shapes: GHOST until `MarowakAnim`, or for the whole fight.
var _gen1_ghost: StringName = &""
var _enemy_ghosted: bool = false
var _unveil: Dictionary = {}
...
## `wAttrmap` bit 3, set over that square by `PokeAnim_SetVBank1` and cleared by
## `PokeAnim_SetVBank0` in `PokeAnim_DeinitFrames`. Bank 1 holds only the enemy's picture
## and frames, so the animation's tile numbers may overlap the player's.
...
## Whether the box holds a line not yet pressed past. Only a box waits for a button in
## `DoMove`'s loop, so this tells the frame pump that its finished run owes a press rather
## than the next command; see [method _resume_after_frames].
...
## Generation 2's pack screen, and the world whose bag it lists.
var _pack_host: Gen2StartMenuScreen = null:
	set(value):
		_pack_host = value
		_list_state_changed()
var _pack_world: Gen2WorldAPI = null
...
## Where a level-up's move offer is, per LearnMove and ForgetMove (engine/pokemon/learn.asm):
## [code]&"ask"[/code] AskForgetMoveText's yes/no, [code]&"list"[/code] ForgetMove's .loop,
## [code]&"stop"[/code] LearnMove.cancel's StopLearningMoveText; empty when none is pending.
...
var _forget_confirm: Gen2WorldMenu = Gen2WorldMenu.yes_no()
...
var _switch_stage: StringName = &"":
	set(value):
		_switch_stage = value
		_gate_annotations()
## Which question the list answers: `OfferSwitch`'s offer, Baton Pass, a faint's
...
var _switch_action: Gen2WorldMenu = null
var _battle_stats: Gen2MonStatsScreen = null
...
## opens on whatever was chosen last, until the next player send-out.
var _menu_position: int = Gen2BattleMenu.FIGHT
## Generation 1's `wCurrentMenuItem` per position, left column first.
const GEN1_SAVED_MENU_ITEM: Dictionary = {
	Gen2BattleMenu.FIGHT: 0, Gen2BattleMenu.PACK: 1,
	Gen2BattleMenu.PKMN: 2, Gen2BattleMenu.RUN: 3,
}
...
## `wSwappingMove`, the row SELECT marked in the move list, or -1.
var _swapping_move: int = -1
## `wCurEnemyMoveNum` as `AIChooseMove` left it when the turn opened, or -1.
var _enemy_turn_slot: int = -1
...
## The trainer class behind the enemy's moves, or zero for [method show_matchup]'s invented
## pairing, which has no AI flags and falls back to [method _random_slot]. Reset by both,
## set only by [method show_trainer].
...
var _victory_music_spent: bool = false

@onready var _screen: Gen2Screen = %Screen


## Bars drain and the intro slides on hardware frames, as [Gen2WorldAnimation] paces the
...
func _process(delta: float) -> void:
	...
	## The box counts hardware frames while nothing else does, printing, scrolling and blinking.
	## [method frames_running] does not answer for it: a box at a page waits on a press, and a
	## caller draining frames would never stop.
...
func _queue_gen1_faint_sounds(player_side: bool) -> void:
	...
	if not _battle.is_trainer_battle:
		return
	_sound_queue.append({"wait": true, "gen1": true, "index": Gen1Sfx.SFX_FAINT_FALL})
	_sound_queue.append({"wait": true, "gen1": true, "index": Gen1Sfx.SFX_FAINT_THUD})
...
## One turn of `AnimateFrontpic`'s `.loop`: one `SetUpPokeAnim`, one hardware frame. Like
## `PokeAnim_PlaceGraphic` it edits only the tilemap, stamped over the enemy's square.
...
func _ready() -> void:
	...
	## The cartridge has one APU: an opener's driver is handed over so a cry takes the channels
	## the map music holds rather than a second copy. Only a standalone battle builds and owns one.
	...
	## one, so the menu covers the box's right half and `MoveSelectionScreen`'s list all of it.
	...
	## Just below the ink, so a placement asking for a field gets the cartridge's interface
	## behind exactly its cells. The built-in arena is already white there; a native-layer
	## renderer has the map there, at its own interface opacity.
...
## `CleanUpBattleRAM` zeroes `wLowHealthAlarm`. A driver this screen does not own outlives
## it, so the byte is cleared on leaving the tree, which every way out of a battle does.
...
## Seeds the enemy's move choice, its item and switch decisions, and the capture rolls. A
## battle in a walk draws from the world's generator so a replay reproduces it unrecorded;
## one with no world seeds itself randomly.
...
func _dude_auto_input_stage() -> StringName:
	if _pack_selecting or _pack_host != null:
		return &"pack"
	if _menu_stage == &"main":
		return &"menu"
...
## `wSimulatedJoypadStates`, so a held A does nothing while the tutor chooses. A press that
## reached the menu fought the demo with the tutor's party and wrote it over the player's.
...
## One hardware frame of everything this screen counts, including the switch menu's party
## icons that [method _process] otherwise paces off real time. Lets the world spend a
## battle's frames from its own pump, so a replay reaches the same place on the same frame.
func advance_hardware_frame() -> bool:
	...
	elif _battle_stats != null:
		_battle_stats.advance_animation()
		_draw_battle_stats()
	elif _switch_stage == &"item_result":
		_advance_item_result()
...
func _advance_yes_no_holds() -> void:
	if _forget_stage in [&"ask", &"stop"] and _forget_confirm.advance_hold():
		_confirm_forget_stage()
	elif _switch_offer != null and _switch_offer.advance_hold():
		_answer_held_switch_yes_no(_switch_offer.answered_yes())
...
func show_trainer(
	...
	show_message(_wants_to_battle_text())
...
func _begin_world_battle(prepared: Dictionary, save: Gen2SaveData) -> void:
	...
	_gen1_ghost = StringName(_world_battle_request.get("gen1_ghost", &""))
	_enemy_ghosted = _gen1_ghost != &""
	_unveil = {}
	_world_battle_completion_sent = false
	_world_battle_result_picture_shown = false
	_world_battle_terminal_text_shown = false
	...
	_battle.first_slot_speed = Gen2SaveBattleAdapter.first_slot_speed(_data, save)
...
## Screenshot driver for the overworld loss boundary: a real host battle with the fallback
## party, completed as a loss on the frame `LostBattle` returns. The whiteout is the overworld's.
...
## A fallback party led by [param species], the species after it behind.
...
## Whether `BattleStartMessage` and `DoBattle`'s opening still owe a line, frames or a ball
## throw. A driver settling to the first menu waits on this and [method frames_running],
## since two of the steps are a box rather than frames.
...
func _init_battle_display() -> void:
	...
	_hud_balls = []
	_hud_border = []
	## `InitBattleDisplay` clears both panels, and the pics slide in as the battlers
	## themselves: `InitEnemyTrainer` places a trainer class on the enemy's square and
	## `GetTrainerBackpic` the player's on the player's. A wild opponent already stands there.
	_enemy_hud_visible = false
	_player_hud_visible = false
...
## `BattleStartMessage` and `DoBattle`'s opening, one stage per [method _advance_entrance]
## step, measured frame by frame on a cartridge. The enemy's cry is `AnimateFrontpic`'s on
## Crystal and `PlayStereoCry`'s (`.cry_no_anim`) on pokegold; the player's has no animation.
func _build_entrance() -> void:
	...
	if trainer:
		_entrance_stages.append({
			"sfx": Gen2Sfx.SFX_SHINE, "wait_sfx": true, "delay": TRAINER_START_FRAMES,
		})
	elif not _enemy_ghosted:  ## `.noSilphScope` and `.isMarowak` play no cry.
	...
	if _enemy_ghosted:
		_entrance_stages.append_array(_ghost_entrance_stages())
	# `EmptyBattleTextbox` and `ClearSprites` take the balls away past that line;
	# the wild branch draws the enemy's panel there too.
	_entrance_stages.append({
...
## `PrintBeginningBattleText`'s `.pokemonTower`: `GhostCantBeIDdText` behind the
## line, or `UnveiledGhostText`, `MarowakAnim` and `WildMonAppearedText`.
func _ghost_entrance_stages() -> Array:
	if _gen1_ghost != Gen1Layout.GHOST_UNVEILED:
		return [{"message": GHOST_CANT_BE_IDD_TEXT}]
	return [
		{"message": GHOST_UNVEILED_TEXT},
		{"unveil": true},
		{"message": "Wild %s\nappeared!" % _name_of(_enemy)},
	]


## One frame of `MarowakAnim`: `FlashSprite8Times` on `rOBP1`, the `sla`s
## shifting it out, the `srl`/`rra` pairs shifting $e4 back in over the MAROWAK.
func advance_unveil() -> bool:
	if _unveil.is_empty():
		return false
	var frame: int = int(_unveil["frame"]) + 1
	_unveil["frame"] = frame
	if frame % UNVEIL_STEP_FRAMES == 0:
		@warning_ignore("integer_division")
		var step: int = frame / UNVEIL_STEP_FRAMES
		if step <= UNVEIL_FLASHES:
			_unveil["dmg"] = int(_unveil["dmg"]) ^ UNVEIL_FLASH_MASK
		elif step <= UNVEIL_FLASHES + UNVEIL_FADE_STEPS:
			_unveil["dmg"] = (int(_unveil["dmg"]) << 2) & 0xFF
			_enemy_ghosted = int(_unveil["dmg"]) != 0
		else:
			var shifted: int = UNVEIL_FADE_IN_STEPS - (step - UNVEIL_FLASHES - UNVEIL_FADE_STEPS)
			_unveil["dmg"] = (Gen2BattleRenderer.PALETTE_IDENTITY << (2 * shifted)) & 0xFF
	if frame >= UNVEIL_FRAMES:
		_unveil = {}
	_push_view()
	return true


## Moves the enemy's cry out of [param stage]'s events into `AnimateFrontpic`, where the
## cartridge plays it. No cached animation or the battle scene off leaves the stage as the
## source's own `.cry_no_anim`.
...
func _advance_entrance() -> bool:
	...
	while not _entrance_stages.is_empty():
		...
		if stage.has("unveil"):
			stage.erase("unveil")
			_unveil = {"frame": 0, "dmg": Gen2BattleRenderer.PALETTE_IDENTITY}
			return true
...
## `WantsToBattleText`, or `_TrainerWantsToFightText` on Generation 1.
func _wants_to_battle_text() -> String:
	return ("%s wants\nto fight!" if _generation() == RomRegistry.GEN1 else "%s\nwants to battle!") \
		% _enemy_battler_label()


## `PlaceEnemysName`: the class, a space and the party row's name, or the name
## the request carries alone: a link partner's, the rival's, Generation 1's.
func _enemy_battler_label() -> String:
	...
	if _enemy_trainer_class <= 0 or (_battle != null and _battle.is_link_battle) \
		or _data.generation == RomRegistry.GEN1 or _rival_class():
		return named if not named.is_empty() else "Enemy"
	return "%s %s" % [
...
func _rival_class() -> bool:
	return _enemy_trainer_class in [Gen2Battle.TRAINER_CLASS_RIVAL1, Gen2Battle.TRAINER_CLASS_RIVAL2]
...
## `SUBSTATUS_SUBSTITUTE` before `wPlayerMinimized`, so a doll hides the dot until it comes
## off. Written by [constant Gen2Battle.MINIMIZED], cleared by a send-out.
...
func advance_animation() -> bool:
	...
	if _anim_delay > 0:
		...
		return true
	if _anim != null:
		# A script's last frame still carries commands: `SubstituteAnim` ends on one.
		var played: bool = _anim.advance_frame()
		if played:
			_after_anim_frame()
		if played and not _anim.finished():
			return true
		_end_script()
		return true
	_run_next_anim_step()
...
func _begin_animation(event: Dictionary) -> void:
	...
	# `PlayFXAnimID`'s `ld c, 3 / call DelayFrames`, then `_PlayBattleAnim`'s six,
	...
	while not _anim_plan.is_empty():
		...
		match StringName(step["kind"]):
			ANIM_DELAY:
				...
				return
			ANIM_CLEAR_HUD:
				# `BattleAnimClearHud`: a delay, the hud off the map, then three more while
				# it reaches VRAM. `ClearActorHud` clears only `hBattleTurn`'s panel, so the
				# target's bar stays to drain.
...
func _gen1_send_out_steps(enemy_turn: bool, param: int = Gen2Battle.SEND_OUT_ANIM_NORMAL) -> void:
	if param == Gen2Battle.SEND_OUT_ANIM_SHINY:
		_step(ANIM_SFX, {"sfx": Gen2Sfx.SFX_SHINE})
...
## `TossBallAnimation`: the ball's throw, then `.PokeBallAnimations` for as many entries as
## `wPokeBallAnimData`'s high nybble names (four on a capture, two on a ball that never
## opens, six on a break-out); a trainer battle takes `.BlockBall`.
func _gen1_toss_ball_steps(event: Dictionary) -> void:
	...
	## `wPokeBallAnimData` $10: the toss alone.
...
func _play_victory_music(event: Dictionary) -> void:
	if _victory_music_spent or _audio_player == null or _data == null \
			or _battle == null or bool(event.get("fled", false)) \
			or event.get("winner", Gen2Battle.PLAYER) != Gen2Battle.PLAYER or _battle.is_link_battle:
		return
	_victory_music_spent = true
	if bool(event.get("silent", false)):
...
func _play_hit_sound() -> void:
	...
	if effectiveness == 0:
		return
	var sfx: int = Gen2Sfx.SFX_DAMAGE
	if effectiveness > Gen2Layout.MATCHUP_EFFECTIVE:
		sfx = Gen2Sfx.SFX_SUPER_EFFECTIVE
	elif effectiveness < Gen2Layout.MATCHUP_EFFECTIVE:
		sfx = Gen2Sfx.SFX_NOT_VERY_EFFECTIVE
	_play_sfx(sfx, true)
...
## How full the exp bar is, in `PlaceExpBar`'s pixels. The committed value: the animation
## draws its own while it runs, and moving the count cancels it as [method set_hp] does.
...
## The animation layer's state for a scene test or screenshot driver: which animation, whose
## turn, whether a script runs (a battle scene off spends the delays only), and how much it draws.
...
func info_snapshot() -> Dictionary:
	...
	for row: Dictionary in _move_rows:
		...
		out["effectiveness"] = Gen2MoveEffect.applied_effectiveness(
			_data.type_effectiveness(int(row.get("type", 0)), defending, identified),
			_data.move(int(row.get("move", 0))), _data.generation
	...
	return {
		...
		"move_cursor": _move_cursor,
		"move_rows": rows,
		## `MoveSelectionScreen`'s first row, row spacing, and the box's right-hand column (the
		## one cell nothing else writes), so a provider annotating the list needs no coordinates.
...
## Whether the annotation layer may draw. Hidden while a full-screen subflow owns the cells:
## party page, pack and its sub-lists, ball selection, forget offer, naming prompt, entrance.
func _annotations_visible() -> bool:
	return _renderer_ready and _annotations != null and _intro == null \
		...
		and not _pack_selecting and _pack_host == null and not _pack_move_selecting \
		and not _capture_selecting and _forget_stage == &""


## One of the lists in front of the fight opened or closed. Both layers follow
## the state, since no message redraws a menu.
func _list_state_changed() -> void:
	_gate_annotations()
	_reopen_menu_layer()


## Rebuilds the layer the frame a modal takes or returns the interface. Every state
## [method _annotations_visible] reads has a setter that reaches here.
...
## The field layer under the ink: the flagged placements' cells in the cartridge's box white,
## at the renderer's interface opacity (1.0 for the built-in arena and the hardware viewport).
...
## Generation 1's `DisplayBagMenu` rows, and a worldless battle's bag.
...
func set_pack_world(world: Gen2WorldAPI) -> void:
	_pack_world = world
...
## `BattleMenu_Pack`: `BattlePack`, or Generation 1's `DisplayBagMenu`.
...
func _open_pack_host() -> void:
	var host: Gen2StartMenuScreen = START_MENU_SCENE.instantiate() as Gen2StartMenuScreen
	var tutorial: bool = _world_battle_tutorial
	var world: Gen2WorldAPI = null if tutorial else _pack_world
	_pack_host = host
	host.z_index = 5
	add_child(host)
	host.set_screen(_screen)
	host.sfx_requested.connect(_play_sfx)
	host.closed.connect(_on_pack_host_closed)
	host.battle_item_chosen.connect(_on_pack_item_chosen)
	var bag: Dictionary = _capture_quantities.duplicate()
	bag.merge(_pack_quantities, true)
	host.open_battle_pack(_data, world, [] if world != null else Gen2WorldPack.build(
		_data, Gen2WorldState.new({}, {}, bag, {})
	), tutorial)


func _close_pack_host() -> void:
	if _pack_host != null:
		Gen2Screen.drop(_pack_host)
		_pack_host = null


## `.didnt_use_item`, which redraws the fight and is `jp BattleMenu`.
func _on_pack_host_closed() -> void:
	_close_pack_host()
	_open_battle_menu()


## `BattleMenu_Pack.tutorial` throws a POKE BALL whatever `TutorialPack` ended on.
func _on_pack_item_chosen(item: int) -> void:
	_close_pack_host()
	if _world_battle_tutorial:
		_throw_ball(_tutor_ball(), &"pack")
		return
	_use_chosen_item(item)


## A refusal over the target's party list (`WontHaveAnyEffectMessage`) or the pack.
func _refuse_in_pack(message: String, target: int = -1) -> void:
	if target < 0 and _generation() == RomRegistry.GEN1:
		show_message(message)
		_pack_selecting = true
		return
	if target < 0:
		_open_pack_host()
		_pack_host.say(message)
		return
	_open_switch_pick(&"item")
	for row: int in _switch_menu.rows.size():
		if int((_switch_menu.rows[row] as Dictionary).get("index", -1)) == target:
			_switch_menu.cursor = row
	_show_switch_refusal(message)


func selected_pack_item() -> int:
	if _pack_rows.is_empty() or _pack_index >= _pack_rows.size():
		return 0
	return int(_pack_rows[posmod(_pack_index, _pack_rows.size())])


## Generation 1's list carries CANCEL after the last item and stops at either end.
func select_pack_row(index: int) -> Dictionary:
	...
	_pack_index = clampi(index, 0, _pack_rows.size())
	_show_pack_selection()
	return {"ok": true, "item": selected_pack_item()}
...
func use_selected_pack_item() -> Dictionary:
	if not _pack_selecting or _battle == null:
		return {"ok": false, "reason": &"pack_not_open"}
	return _use_chosen_item(selected_pack_item())


func _use_chosen_item(item: int) -> Dictionary:
...
## `UseBallInTrainerBattle`, where `PokeBallEffect` jumps before saying anything: no ITEM USED
## line, the throw on `BattleAnim_ThrowPokeBall`'s NO_ITEM branch, two boxes, then
## `UseDisposableItem` spends the ball. The turn goes too: `wItemEffectSucceeded` and
## `wBattlePlayerAction` are one byte.
...
func _use_pack_item(item: int, target: int, move_slot: int = -1) -> Dictionary:
	...
	if not bool(used.get("ok", false)):
		...
		_refuse_in_pack(_item_refusal_text(StringName(used.get("reason", &""))), target)
		return used
	## `UseDisposableItem`, which the Poke Flute alone is never handed to.
	if bool(used.get("spent", true)):
		item_used.emit(item, target)
	var effect: Dictionary = used.get("effect", {})
	var row: StringName = Gen2ItemActionText.kind(_data, item, effect) \
		if StringName(used.get("kind", &"")) == &"party_item" else &""
	if row != &"":
...
## `ItemActionTextWaitButton` prints and holds before a press.
...
func _open_item_result(
	...
	for index: int in menu.rows.size():
		if int((menu.rows[index] as Dictionary).get("index", -1)) == target:
			at = index
	...
	if healed > 0:
		...
		_item_result["anim"] = Gen2HpBarAnimation.create(
			to_hp - healed, to_hp, int(target_row.get("max_hp", 0))
		)
	else:
		_play_sfx(Gen2Sfx.SFX_FULL_HEAL)
...
func _advance_item_result() -> void:
	var anim: Gen2HpBarAnimation = _item_result.get("anim", null)
	if anim != null:
		anim.advance_frame()
		if anim.finished():
			_item_result.erase("anim")
	elif int(_item_result.get("hold", 0)) > 0:
		_item_result["hold"] = int(_item_result["hold"]) - 1
	_reopen_menu_layer()


func _answer_item_result(button: int) -> void:
...
func _draw_item_result() -> void:
	if _party_page == null or _item_result.is_empty():
		return
	var rows: Array = (_item_result["rows"] as Array).duplicate(true)
	var anim: Gen2HpBarAnimation = _item_result.get("anim", null)
	if anim != null:
...
func capture_ghost() -> bool:
	return _battle != null and _battle.gen1_ghost
...
## Opens the ball selector of a Generation 1 battle with no bag.
...
## `PokeBallEffect` from `ItemUsedText` on; the world resolves the throw. The turn
## is banked in `wBattlePlayerAction` and a refusal takes it back.
...
func complete_capture(result: Dictionary) -> Dictionary:
	...
	if not bool(result.get("ok", false)):
		## `Ball_BoxIsFullMessage`'s `$2` is `.didnt_use_item`: the pack again, the
		## ball kept and the turn still the player's, like every refusal here.
		_capture_result.clear()
		_capture_spent_turn = {}
		var refusal: String = _ball_box_full_text() \
			if StringName(result.get("reason", &"")) == &"storage_full" \
			else "The capture could not be completed."
		if _capture_origin == &"pack":
			_refuse_in_pack(refusal)
		else:
			show_message(refusal)
			_capture_selecting = not _capture_balls.is_empty()
		_reopen_menu_layer()
		return result
	...
	if caught:
		...
		_box_queue.append(GEN1_DODGED_TEXT)
	else:
		_box_queue.append(
...
func _begin_capture_animation(ball: int, wobbles: int, caught: bool, dodged: bool = false) -> void:
	if ball <= 0:
		return
...
func _item_refusal_text(reason: StringName) -> String:
	...
	return _gen1_item_text(
		"not_time", "OAK: <PLAYER>!\nThis isn't the" + SCROLL + "time to use that!"
...
## `AskGiveNicknameText`, `YesNoBox` and `NamingScreen`, run by `PokeBallEffect` for a catch
## sent to party or box and skipped by `.catch_bug_contest_mon` and `.FinishTutorial`. True
## while the prompt stands.
func _open_capture_nickname() -> bool:
	...
	## when a Nuzlocke skips the question.
...
## `PokeBallEffect` gives no experience: this is a registered policy's addition, none by
## default. The tutorial and a Bug Contest catch (a score, not a kept Pokemon) are excluded.
...
## [Gen2BattleAI] against a trainer's AI flags, or [method _random_slot] for
## [method show_matchup]'s invented pairing, which has no flags to read.
...
## The enemy's move, picked when the turn opened, with its trainer's flags for
## the switch or item the battle asks for inside the turn.
func _enemy_action() -> Dictionary:
	var slot: int = _enemy_turn_slot if _enemy_turn_slot >= 0 and _battle.turn_begun() \
		else _enemy_slot()
	_enemy_turn_slot = -1
	var action: Dictionary = Gen2Battle.use_move(slot)
	if _enemy_trainer_class == 0 or _generation() == RomRegistry.GEN1:
		return action
	var policy: Dictionary = Gen2BattleAI.trainer_policy(
		_data, _enemy_trainer_class, _battle.in_battle_tower
	)
	action["ai_item_switch"] = _rules().ai_item_switch(int(policy["item_switch"]))
	return action


## `BattleTurn` before `BattleMenu`: the opening's lines, then `AIChooseMove`
## (Generation 1 picks behind the player). True while the lines are up.
func _begin_turn() -> bool:
	if _battle.turn_begun():
		return false
	var opening: Array = _battle.begin_turn()
	if _generation() != RomRegistry.GEN1:
		_enemy_turn_slot = _enemy_slot()
	if opening.is_empty():
		return false
	_pending = opening
	_show_next_event()
...
func _show_forget_stage(stage: StringName) -> void:
	_forget_stage = stage
	_forget_confirm = Gen2WorldMenu.yes_no()
	if stage == &"list":
		_show_forget_list()
	else:
...
func _answer_forget(button: int) -> void:
	...
	match _forget_stage:
		&"ask", &"stop":
			if not _offer_still_reading() and _forget_confirm.press_yes_no(button):
				_click_if_answered(_forget_confirm)
				if not _forget_confirm.holding():
					_refresh_menu_layer()
		&"list":
			match button:
				## `StaticMenuJoypad` moves the cursor under the question it printed.
				PokeButton.UP, PokeButton.DOWN:
					_forget_cursor = Gen2MoveForget.step_cursor(_forget_cursor,
						1 if button == PokeButton.DOWN else -1, _forget_moves.size(), _generation())
					_refresh_menu_layer()
				PokeButton.A:
...
func _confirm_forget_stage() -> void:
	var yes: bool = _forget_confirm.answered_yes()
...
func advance() -> void:
	if _box == null or (_link_text_frames > 0 and _message_awaits_press):
		return
	## An exp bar stopped at a level boundary sits under `.LoopLevels`' `StdBattleTextbox`,
	## which blocks on a button: this press lets the loop fill the next level.
...
func _messages_held() -> bool:
	...
	if _intro != null or bars_animating() or fainting() or animation_running():
		return true
	## `applydamage` animates the bar and sinks the picture before `criticaltext` prints, so a
	## line produced meanwhile was held. Released here, where every wait ends, including a faint
	## with no bar, whose frame the bar pump never sees.
...
func _continue_capture() -> void:
	...
	if _open_capture_nickname():
		return
	## A catch leaves `wBattleResult` at WIN, so `CheckPayDay` still runs.
...
## The turn a throw paid for. `.UseItem` returns no carry on a ball that did not land, so
## `BattleMenu` falls into `DoBattle` and the enemy moves; only a catch escapes through `.run`.
## Answers whether that turn owns the screen.
...
func _finish_battle() -> void:
	...
	if _world_battle_active and not _battle.is_draw():
		...
		if _show_world_battle_terminal_text():
			return
		if _show_gen1_blackout_text():
			return
		## `.give_money` is the next thing `BattleWon` does once
		## `PrintWinLossText` has been answered.
		if _show_prize_money_text():
			return
		## `LostBattle`'s `.not_canlose` is the grayscale and a `ret`; `_WhitedOutText` belongs to
		## the overworld's `Script_Whiteout`. This only checks that the party to heal is readable.
...
## The fought party over the live save, nothing written to disk. A catch builds its candidate
## from the live save; without this it would carry the pre-battle party and refund HP and PP.
...
## [method Gen2WorldBattleAdapter.earnings] worked out once, so the saved snapshot, the prize
## line and the world's completion result cannot disagree.
...
func _show_world_battle_result_picture() -> bool:
	...
	if _battle.winner() != Gen2Battle.PLAYER:
		## `HandlePlayerBlackOut`'s OPP_RIVAL1 branch slides the pic in too.
		if not _battle.in_battle_tower and not bool(_world_battle_request.get("can_lose", false)) \
			and not _gen1_rival1_loss():
			return false
		_enemy_hud_visible = false
...
func _show_world_battle_terminal_text() -> bool:
	...
	## `SET_PAL_BATTLE_BLACK` stands behind `Rival1WinText` alone.
	if _gen1_lost() and not _gen1_rival1_loss():
		_set_gen1_black()
	var key: String = (
		"win_text" if _battle.winner() == Gen2Battle.PLAYER else "loss_text"
	)
	## `LostBattle` prints it for `BATTLETYPE_CANLOSE` and the Battle Tower alone.
	if key == "loss_text" and _generation() != RomRegistry.GEN1 and not _battle.in_battle_tower \
			and not bool(_world_battle_request.get("can_lose", false)):
		return false
...
func _gen1_rival1_loss() -> bool:
	return _gen1_lost() and _battle.is_trainer_battle and not _battle.is_link_battle \
		and _enemy_trainer_class == Gen1Layout.RIVAL1_CLASS


func _set_gen1_black() -> void:
	_gen1_black = true
	_push_view()


## `HandlePlayerBlackOut`'s `.notRival1Battle`. A link loss said
## `LinkBattleLostText` in its place; the starter fight in OAKS_LAB returns first.
func _show_gen1_blackout_text() -> bool:
	if _gen1_blackout_shown or not _gen1_lost() or _battle.is_link_battle:
		return false
	_gen1_blackout_shown = true
	if _gen1_rival1_loss() and _world_context != null \
		and _world_context.map_number() == Gen1Layout.OAKS_LAB:
		return false
	_set_gen1_black()
	var text: String = String(_world_battle_request.get("blackout_text", ""))
	if text.is_empty():
		return false
	show_message(text)
	return true
...
## `BattleMenu`, opened over a box `EmptyBattleTextbox` cleared. `CheckPlayerHasUsableMoves`
## runs inside `MoveSelectionScreen`, so a Pokemon with nothing left still opens the menu and
## Struggle is chosen.
func _open_battle_menu() -> void:
	...
	if _battle.awaiting_move_learn() or _battle.must_replace(Gen2Battle.PLAYER):
		return
	## `CheckContestBattleOver`: out of Park Balls is a DRAW.
	if _is_bug_contest_battle() and available_capture_balls().is_empty():
		_battle.force_out(Gen2Battle.PLAYER)
		_pending = [{"type": Gen2Battle.OVER, "winner": _battle.winner()}]
		_show_next_event()
		return
	if _begin_turn():
		return
...
func _open_move_menu() -> void:
	...
	_battle.fight_chosen()
	## Encore and Bide leave before `MoveSelectionScreen`'s usable-move check.
	if _battle.player_move_menu_skipped():
		_close_battle_menu()
		_take_turn_with_slot(_move_cursor)
		return
	_move_rows = Gen2BattleMenu.move_rows(mon, _data)
	if _move_rows.is_empty() or not mon.offers_move_list():
		_close_battle_menu()
		if _battle.is_over():
			return
...
func _answer_battle_menu(button: int) -> void:
	match button:
		PokeButton.UP, PokeButton.DOWN, PokeButton.LEFT, PokeButton.RIGHT:
			...
			_refresh_menu_layer()
		PokeButton.A:
			## `Get2DMenuSelection`'s `MenuClickSound`.
	...
	match _menu_position:
		...
		Gen2BattleMenu.PACK:
			...
			if not _battle.allows_bag_items() or not _pack_rows.is_empty() \
				or _generation() != RomRegistry.GEN1 or _world_battle_active:
				open_battle_pack()
				return
			## A Generation 1 battle with no bag still reaches the throw.
			if _is_wild_battle():
				begin_capture()
				return
...
func _answer_move_menu(button: int) -> void:
	match button:
		PokeButton.UP, PokeButton.DOWN:
			...
			_refresh_menu_layer()
		## `ParsePlayerAction`'s `PlayClickSFX` either way; Generation 1 clicks
		## on every A.
		PokeButton.SELECT:
			_press_move_select()
		PokeButton.B:
		...
		PokeButton.A:
			...
			if refusal == &"" or _generation() == RomRegistry.GEN1:
				_play_sfx(Gen2Sfx.SFX_READ_TEXT_2)
			if refusal != &"":
				_menu_stage = &"refused"
...
## `.pressed_select`, outside a link: mark a row, then swap it with the cursor's.
func _press_move_select() -> void:
	if _battle.is_link_battle or _generation() == RomRegistry.GEN1:
		return
	if _swapping_move < 0:
		_swapping_move = _move_cursor
	else:
		_battle.mon(Gen2Battle.PLAYER).swap_moves(_swapping_move, _move_cursor)
		_swapping_move = -1
		_move_rows = Gen2BattleMenu.move_rows(_battle.mon(Gen2Battle.PLAYER), _data)
	_refresh_menu_layer()
...
## Opens `OfferSwitch`'s yes/no, which only SHIFT reaches, and answers whether it did. Asked
## before a replacement, as a Baton Pass is: the stopped turn has not finished.
...
func _open_switch_offer() -> void:
	...
	_open_yes_no(&"offer", Gen2BattleSwitchMenu.offer_text(
		_enemy_label(), incoming.display_name() if incoming != null else "", _player_label(),
...
func _open_yes_no(stage: StringName, question: String) -> void:
	...
	_switch_offer = Gen2WorldMenu.yes_no()
	_switch_stage = stage
	_reopen_menu_layer()
...
func _answer_switch(button: int) -> void:
	match _switch_stage:
		&"offer", &"use_next", &"contest_replace":
			_answer_switch_yes_no(button)
		&"pick":
			_answer_switch_pick(button)
		&"action":
			_answer_switch_action(button)
		&"item_result":
			_answer_item_result(button)
		&"stats":
			_battle_stats.handle_button(button)
			if _battle_stats != null:
				_draw_battle_stats()
		&"refused":
			## `jr .loop` reopens the list; an item's `.BattleField` the pack.
			if _box != null and _box.advance():
				return
...
## `BugContest_SetCaughtContestMon`'s `PlaceYesNoBox` under `_ContestAskSwitchText`; `ret c`
## keeps the Pokemon already caught. The answer rides out on the capture result, as world state.
...
## `InterpretTwoOptionMenu` for `OfferSwitch`, `AskUseNextPokemon` and the
## contest's replace question, whose box is up only once the text is read.
func _answer_switch_yes_no(button: int) -> void:
	if _offer_still_reading():
		if button == PokeButton.A:
			_box.advance()
			_refresh_menu_layer()
		return
	if _switch_offer.press_yes_no(button):
		_click_if_answered(_switch_offer)
		if not _switch_offer.holding():
			_refresh_menu_layer()
...
func _answer_held_switch_yes_no(yes: bool) -> void:
	match _switch_stage:
		&"offer":
			if yes:
...
## `PartyMenuSelect`'s joypad filter: the cursor, and A and B with their click.
	...
	match button:
		...
		PokeButton.A:
			...
			if _switch_reason == &"item" and not _switch_menu.is_cancel(_switch_menu.cursor):
				...
				return
			if _switch_reason == &"player" and not _switch_menu.is_cancel(_switch_menu.cursor):
				_switch_action = Gen2BattleSwitchMenu.action_menu()
				_switch_stage = &"action"
				_reopen_menu_layer()
				return
...
## `BattleMonMenu`: SWITCH is `TryPlayerSwitch`, STATS `Battle_StatsScreen`,
## CANCEL `.Cancel` and B `.PressedB` back to the list.
func _answer_switch_action(button: int) -> void:
	match button:
		PokeButton.UP, PokeButton.DOWN:
			if _switch_action.move(Vector2i(0, 1 if button == PokeButton.DOWN else -1)):
				_refresh_menu_layer()
		PokeButton.B:
			_play_sfx(Gen2Sfx.SFX_READ_TEXT_2)
			_switch_stage = &"pick"
		...
		PokeButton.A:
			_play_sfx(Gen2Sfx.SFX_READ_TEXT_2)
			match _switch_action.selected_index():
				0:
					_switch_stage = &"pick"
					_resolve_switch(_player_switch_answer())
				1:
					_open_battle_stats()
				_:
					_close_switch()
...
## `TryPlayerSwitch`: already out, trapped, then fitness, each back on the list.
func _player_switch_answer() -> Dictionary:
	var answer: Dictionary = _switch_menu.confirm()
	if _switch_reason != &"player" or not _battle.switch_blocked() \
			or StringName(answer.get("result", &"")) == Gen2BattleSwitchMenu.ALREADY_OUT \
			or _switch_menu.is_cancel(_switch_menu.cursor):
		return answer
	return {"result": &"trapped", "text": _battle_line(
		BATTLE_TEXT[Gen2Battle.SWITCH_BLOCKED], {"side": Gen2Battle.PLAYER}
	)}


## `Battle_StatsScreen` over the party as the fight has left it.
func _open_battle_stats() -> void:
	var party: Gen2SaveData = Gen2SaveBattleAdapter.from_battle_party(
		_data.id, _data.sha1, -1, _battle.party(Gen2Battle.PLAYER), "", _source_save
	)
	if party == null or party.party.is_empty():
		_switch_stage = &"pick"
		_reopen_menu_layer()
		return
	_battle_stats = Gen2MonStatsScreen.create(
		_data, party.party, _switch_menu.cursor, party
	)
	_battle_stats.closed.connect(_close_battle_stats)
...
## `BattleMenuPKMN_ReturnFromStats`, on the row the stats screen walked to.
func _close_battle_stats() -> void:
	_switch_menu.cursor = clampi(_battle_stats.cursor(), 0, _switch_menu.rows.size() - 1)
	_battle_stats = null
	_switch_stage = &"pick"
	_reopen_menu_layer()


func _draw_battle_stats() -> void:
	if _stats_page == null:
		_stats_page = Gen2StatsScreenPage.from_data(_data)
	if _stats_page != null:
		_show_menu_image(Gen2StatsScreenPage.compose(_stats_page, _data, _battle_stats), Vector2i.ZERO)


func _draw_switch_action() -> void:
	if _menu_page == null:
		return
	var at: Rect2i = Gen2BattleSwitchMenu.ACTION_BOX
	var box: Gen2MenuBox = Gen2MenuBox.from_coords(
		at.position.x, at.position.y, at.end.x, at.end.y, _switch_action.flags
	)
	_show_layer_image(
		_info_layer, _menu_page.render(box, _switch_action.options, _switch_action.cursor),
		box.border_position() * Gen2Font.TILE
	)
...
func _menu_signature() -> String:
	return "%s|%s|%d|%d|%d|%s|%s|%s" % [
		...
		"%s%d,%d,%d,%d,%d,%d" % [
			_forget_stage, _forget_cursor, _forget_confirm.cursor,
		...
		"%s%d%d" % [_pack_action_stage, _pack_action_index,
			_switch_action.cursor if _switch_action != null else -1],
...
func _draw_move_menu() -> void:
	...
	## `.battle_player_moves`' `'▷'` at the marked row.
	var extras: Array = [] if _swapping_move < 0 \
		else [{"text": "▷", "at": box.cursor_position(_swapping_move)}]
	_show_layer_image(
		_battle_menu_layer,
		_menu_page.render(box, names, _move_cursor, "", 0, extras),
		box.border_position() * Gen2Font.TILE
	)
...
func _draw_list_menu(key: StringName, rows: Array, cursor: int) -> void:
	...
	var box: Gen2MenuBox = Gen2BattleMenu.list_box(scroll, labels.size())
	_show_layer_image(
		_battle_menu_layer,
		_menu_page.render(
			box,
...
## `DisplayBagMenu`'s list, the START menu bag's rows with CANCEL last.
func _draw_gen1_pack_menu() -> void:
	var entries: Array = []
	for item: int in _pack_rows:
		entries.append({
			"item": item, "name": _item_name(item), "quantity": int(_pack_quantities.get(item, 1)),
		})
	var scroll: int = Gen2BattleMenu.list_scrolled(
		int(_list_scroll.get(&"pack", 0)), _pack_index, entries.size() + 1,
	...
	if _mart_page != null:
		_show_layer_image(_battle_menu_layer, _mart_page.render_gen1_pack({
			"rows": Gen2WorldPack.list_rows(_data, Gen1Layout.BAG_POCKET, entries, scroll, true,
				Gen2MartPage.GEN1_LIST_HEIGHT),
			"cursor": _pack_index - scroll,
		}), Vector2i.ZERO)


## The small ball selector a fight with no bag behind it is handed.
...
## The USE/QUIT box over the ball selector, on [member _info_layer] so it covers
...
func _draw_forget_stage() -> void:
	if _forget_stage != &"list":
		_draw_yes_no_box(_forget_confirm.cursor)
		return
	_menu_layer.visible = false
...
## `HandlePlayerMonFaint` and `HandleEnemyMonFaint`'s replacement tail, and whether any had
## work, in source order: `AskUseNextPokemon`'s wild question, `ForcePlayerMonChoice`'s list,
## and the trainer's entrance, which SHIFT turns into another offer.
...
## The next event, with its changes applied first. Numbers come from the event, not the
## Pokémon: the turn resolved already, so the Pokémon holds the turn's end.
func _show_next_event() -> void:
	while not _pending.is_empty():
		...
		if StringName(event["type"]) == Gen2Battle.CRY:
			## `PlayStereoCry` is `_PlayMonCry` then `WaitSFX`, so an entrance waits out the cry.
			## The silent `PlayStereoCry2` is the pic animation's.
			_apply_event(event)
			_anim_plan = []
			_step(ANIM_WAIT_SFX, {})
			_run_next_anim_step()
...
## `AnimateHPBar` and `MonFaintedAnimation` block until the bar empties and
## the picture sinks; the hit's own lines are held behind them.
func _waits_on_bars() -> bool:
	if _bars.is_empty() and not fainting():
		return false
	if not _pending.is_empty() and HIT_LINE_EVENTS.has(_pending[0]["type"]):
		var line: Dictionary = _pending.pop_front()
		_held_message = _describe(line)
		_held_prompt = not _line_runs_on(line)
	return true


## `applydamage`'s Endure lines, `criticaltext` and `supereffectivetext`.
const HIT_LINE_EVENTS: Array[StringName] = [
	Gen2Battle.ENDURED_HIT, Gen2Battle.ENDURED, Gen2Battle.CRITICAL_HIT, Gen2Battle.EFFECTIVENESS,
]
...
const EVENT_STATE_HANDLERS: Dictionary = {
	Gen2Battle.VICTORY_MUSIC: &"_play_victory_music",
...
## The START menu's row after the battle, one byte with the battle menu's:
## `CleanUpBattleRAM` zeroes it, and Generation 1 keeps the last saved.
func start_menu_cursor_after() -> int:
	if _generation() != RomRegistry.GEN1:
		return 0
...
func _apply_sent_out(event: Dictionary) -> void:
	...
	if side == Gen2Battle.ENEMY:
		...
		## `SendOutPlayerMon` (and `SendOutMon`) zero both cursors.
		_menu_position = Gen2BattleMenu.FIGHT
		_move_cursor = 0
	...
	match event["type"]:
		...
		Gen2Battle.TRANSFORMED:
			...
			if int(event["side"]) == Gen2Battle.ENEMY:
				...
				_player_unown_form = int(event.get("unown_form", 0))
				_player_shiny = bool(event.get("shiny", false))
				## Both generations' Transform zero the move cursor.
				_move_cursor = 0
			_push_view()
		Gen2Battle.ENCORE_INFLICTED:
			## `BattleCommand_Encore.got_player_move` writes `wCurMoveNum`.
			if int(event.get("target", -1)) == Gen2Battle.PLAYER:
				_move_cursor = int(event.get("slot", _move_cursor))
		Gen2Battle.CRY:
		...
		Gen2Battle.GREW_LEVEL:
			# The panel's level is the active mon's, so it moves only for that index; a benched
			# participant has no panel. The bar is not recomputed either: `.LoopLevels` is inside
			# `AnimateExpBar`, which owns it until the walk ends.
			...
			_play_sfx(Gen2Sfx.SFX_HIT_END_OF_EXP_BAR)
...
## Generation 2's lines, `data/text/battle.asm` by label: [label, values...]. `<USER>` is the
## event's `side`, `<TARGET>` its `target` or the other side, unless `user:`/`target:` names a
## field; each other value fills the next `text_ram` or `text_decimal` marker.
const BATTLE_TEXT: Dictionary = {
	Gen2Battle.MISSED: ["AttackMissedText"],
	Gen2Battle.NO_EFFECT: ["DoesntAffectText"],
	Gen2Battle.UNAFFECTED: ["UnaffectedText"],
	Gen2Battle.IT_FAILED: ["ItFailedText"],
	...
	Gen2Battle.MIRROR_MOVE_FAILED: ["MirrorMoveFailedText"],
	Gen2Battle.ATTACK_MAXIMIZED: ["BellyDrumText"],
	Gen2Battle.IGNORED_ORDERS: ["IgnoredOrders2Text"],
...
const GEN1_LINES: Dictionary = {
	...
	Gen2Battle.CONFUSE_INFLICTED: ["%s\nbecame confused!", &"name:target"],
	Gen2Battle.CONFUSED: ["%s\nis confused!", &"name:side"],
	Gen2Battle.SNAPPED_OUT: ["%s's\nconfused no more!", &"name:side"],
	...
	Gen2Battle.MIRROR_MOVE_FAILED: ["The MIRROR MOVE\nfailed!"],
	Gen2Battle.BIDE_STORING: ["%s\nis saving energy!", &"name:side"],
	Gen2Battle.BIDE_UNLEASHED: ["%s\nunleashed energy!", &"name:side"],
	Gen2Battle.RAGE_BUILDING: ["%s's\nRAGE is building!", &"name:target"],
	Gen2Battle.MIMIC_LEARNED: ["%s\nlearned" + SCROLL + "%s!", &"name:side", &"move:move"],
	Gen2Battle.TYPE_COPIED: ["Converted type to\n%s's!", &"name:target"],
	Gen2Battle.DISABLE_INFLICTED: ["%s's\n%s was" + SCROLL + "disabled!", &"name:target", &"move:move"],
	Gen2Battle.DISABLE_ENDED: ["%s's\ndisabled no more!", &"name:side"],
	Gen2Battle.HP_RESTORED: ["%s\nregained health!", &"name:side"],
	Gen2Battle.HP_ALREADY_FULL: ["But, it failed!"],
	Gen2Battle.WENT_TO_SLEEP: ["%s\nstarted sleeping!", &"name:side"],
	Gen2Battle.RESTED: ["%s\nfell asleep and" + SCROLL + "became healthy!", &"name:side"],
	Gen2Battle.NOTHING_HAPPENED: ["No effect!"],
	Gen2Battle.CRASHED: ["%s\nkept going and" + SCROLL + "crashed!", &"name:side"],
	Gen2Battle.SUBSTITUTE_MADE: ["It created a\nSUBSTITUTE!"],
	Gen2Battle.SUBSTITUTE_ALREADY: ["%s\nhas a SUBSTITUTE!", &"name:side"],
	Gen2Battle.SUBSTITUTE_TOO_WEAK: ["Too weak to make\na SUBSTITUTE!"],
	Gen2Battle.SUBSTITUTE_TOOK_DAMAGE: [
		"The SUBSTITUTE\ntook damage for" + SCROLL + "%s!", &"name:target",
	],
	Gen2Battle.SUBSTITUTE_FADED: ["%s's\nSUBSTITUTE broke!", &"name:target"],
	...
	Gen2Battle.FLED_IN_FEAR: ["%s\nran away scared!", &"name:target"],
	Gen2Battle.BLOWN_AWAY: ["%s\nran away scared!", &"name:target"],
	## `TrappingEffect` and `HandleScreens` say nothing of their own.
	Gen2Battle.TRAPPED: [""],
...
const LINE_HANDLERS: Dictionary = {
	Gen2Battle.NO_PP_LEFT: &"_no_pp_left_text",
	Gen2Battle.CRITICAL_HIT: &"_critical_text",
	Gen2Battle.EFFECTIVENESS: &"_effectiveness_text",
...
func _no_pp_left_text(event: Dictionary) -> String:
	return _battle_line([
		"HasNoPPLeftText" if bool(event.get("continuous", false)) else "NoPPLeftText",
		&"move:move",
	], event)
...
func _critical_text(event: Dictionary) -> String:
	if bool(event.get("ohko", false)):
		return "One-hit KO!" if _generation() == RomRegistry.GEN1 \
			else _battle_text("OneHitKOText", event)
	return _hit_line("CriticalHitText", event)


func _effectiveness_text(event: Dictionary) -> String:
	return _hit_line("SuperEffectiveText" if int(event["effectiveness"]) \
		> Gen2Layout.MATCHUP_EFFECTIVE else "NotVeryEffectiveText", event)


func _hit_line(label: String, event: Dictionary) -> String:
	if _generation() == RomRegistry.GEN1:
		return String(GEN1_HIT_TEXT[label])
	return _battle_text(label, event)


## `GainedText` with `WithExpAllText` on the `wBoostExpByExpAll` pass and
## `BoostedText` on a traded learner's; Crystal has the second alone.
func _exp_gained_text(event: Dictionary) -> String:
	var learner: String = _event_name(event)
	var boosted: bool = bool(event.get("boosted", false))
	if bool(event.get("exp_share", false)) and _generation() == RomRegistry.GEN1:
...
## `_MultiHitText` on the player's attack and `_HitXTimesText` on the enemy's.
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if _renderer_input_free():
		...
		if not action.is_empty():
			...
			accept_event()
			return
	# What reaches here is unclaimed input a renderer may use; Gen2WorldScreen offers its
	# renderer the same leftovers so a self-composing shot can be steered.
	if _renderer_input_free() \
...
func _handle_button(button: int) -> bool:
	...
	if _pack_host != null:
		return _pack_host.handle_button(button)
	for row: Array in [
...
## The text box over a native-layer renderer, the seam
## [method Gen2WorldScreen._apply_renderer_interface_style] opens on the map: the renderer
## sets the field's opacity and is told where the screen's box is.
...
## Hands the renderer the battle's location when supplied and asked for. After
## set_battle_data() and before the first view; a renderer swapped in mid-battle gets it again.
...
## Says what a view switch did, through the refusals' log, unless a menu holds
## the interface and a line under it would leak out beside the list.
...
## Whether a menu of the interface holds the lower panel. Unlike
## [method _annotations_visible] (full-screen subflows), the main and move menus own the
## panel a message prints into while the annotations stay correct.
...
func _push_view() -> void:
	...
	var view: Dictionary = {
		...
		## And whose is Minimize's dot: see [method Gen2BattleRenderer.square_pixels].
		...
		## Who the fight is against. A wild battle carries class 0 and an empty name, as
		## `wOtherTrainerClass` is zero there; the class feeds `GameData.trainer_pic()` and
		## `trainer_name()`, so a renderer can draw the cartridge's picture of them.
		...
		## `CopyBackpic` places the player's back pic before `InitBattleDisplay` slides, so it
		## comes in with the middle band; its top three tile rows fall in the enemy's band,
		## hence these eighteen sprites.
		"intro_sprites": _intro.sprites() if _intro != null else [],
		## `GetSGBLayout SCGB_BATTLE_GRAYSCALE` runs on entry and `SCGB_BATTLE_COLORS` after
		## `BattleIntroSlidingPics`, so the slide is colourless until its last frame.
		"grayscale": _intro != null,
		"gen1_black": _gen1_black,
...
## One side's square: who stands on it, whether on it, how far from resting and how big;
## `docs/MODS.md` documents the block, so a planeless renderer need not rebuild host state.
...
## One axis of the animation's scroll per scanline: `hSCX`/`hSCY` is the base, replaced line
## by line while `hLCDCPointer` names that axis. A table aimed at `rBGP` reaches nothing:
## `UpdatePals` never touches it on Color hardware.
...
func _announce() -> void:
	var wild: String = _enemy_mon_name()
	## `.pokemonTower`: `EnemyAppearedText` in place of the wild line.
	if _enemy_ghosted:
		show_message("%s\nappeared!" % wild)
		return
	match _battle.battle_type if _battle != null else Gen2Battle.BATTLETYPE_NORMAL:
		Gen2Battle.BATTLETYPE_FISH:
			show_message("The hooked\n%s%sattacked!" % [
				wild, SCROLL if _generation() == RomRegistry.GEN1 else "\n",
			])
