...
## Whether the `YesNoBox` is up and unanswered, which is `PrintText` having
## returned. A driver presses the text past its own `cont` while this is false.
func question_ready() -> bool:
	return _yes_no != null and _yes_no.is_open() and not _yes_no.holding()


func answering() -> bool:
	return _yes_no != null and _yes_no.holding()
...
func advance_frame() -> void:
	...
	if _phase == Phase.BEFORE_TEXT:
		...
		return
	## `GiveANickname_YesNo` opens `YesNoBox` once `PrintText` returns.
	if _phase == Phase.ASK and not _text_owes_frames():
		_yes_no.open()
...
func handle_button(button: int) -> bool:
	match _phase:
		...
		Phase.ASK:
			...
			if button == PokeButton.A and _text_owes_frames():
				_text_box.advance()
				return true
		Phase.AFTER_TEXT:
